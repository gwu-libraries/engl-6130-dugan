...
class Notebook:
        ...
        if self.data[self.index]['cell_type'] == 'markdown':
            ...
            for i, line in enumerate(self.data[self.index]['source']):
                if not is_directive and (line.startswith('```{image}') or line.startswith('````{image}')):
                    ...
                    image_url = re.sub(r'^\./', f'{IMAGE_URL}/') # Replace initial relative path with absolute path
                elif is_directive:
    ...
    def remove_admonitions(self):
        ...
        '''
        if self.data[self.index]['cell_type'] == 'markdown' and self.data[self.index]['source'][0].startswith('````{admonition}'):
